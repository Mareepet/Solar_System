...
var Rmercury, Rvenus, Rearth, Rmoon, Rmars, Rjupiter, Rsaturn, Ruranus, Rneptune, Rring;
...
function move() {
  if (!animationStarted) {
    return; // Exit the function if animation hasn't started
}
  moveEarth();
  moveMercury();
  moveVenus();
...
function moveNeptune() {
  ...
  if (deltaNeptune < 180) {
    $neptuneContainer.css('z-index', 18);
  } else {
    $neptuneContainer.css('z-index', 2);
  }
  deltaNeptune += 5.43 / n;
  if (deltaNeptune > 360) {
    deltaNeptune -= 360;
